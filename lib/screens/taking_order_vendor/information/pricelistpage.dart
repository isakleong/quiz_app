        ...
        Obx(()=>Expanded( child: ListView.builder(
          itemBuilder: (c, i) {
            if(_takingOrderVendorController.isSuccess.value){
                var filedir =  _takingOrderVendorController.pricelistdir[i];
                var splitteddir = filedir.split("/");
                var nmfile = splitteddir[splitteddir.length - 1].replaceAll("%20", " ");
                return Padding(
                  ...
                  child: Align(
                    ...
                    child: InkWell(
                      onTap: () async {
                          var ext = "";
                          int lastDotIndex = nmfile.lastIndexOf('.');
                          if (lastDotIndex != -1 && lastDotIndex < nmfile.length - 1) {
                            ext = nmfile.substring(lastDotIndex + 1);
                          }
                          if (await File("${_takingOrderVendorController.productdir}/${filedir.replaceAll("%20", " ")}").exists()) {
                            if (ext == "jpg" || ext == "jpeg") {
                              Get.to(ViewImageScreen("${_takingOrderVendorController.productdir}/${filedir.replaceAll("%20", " ")}"));
                            } else if (ext == "pdf") {
                              Get.to(ViewPDFScreen("${_takingOrderVendorController.productdir}/${filedir.replaceAll("%20", " ")}"));
                            } else if (ext == "mp4") {
                              Get.to(ViewVideoScreen("${_takingOrderVendorController.productdir}/${filedir.replaceAll("%20", " ")}"));
                            }
                          } else {
                            Get.defaultDialog(
                              radius: 6,
          ...
          itemCount: _takingOrderVendorController.isSuccess.value == false ? 10 : _takingOrderVendorController.pricelistdir.isEmpty ? 0 : _takingOrderVendorController.pricelistdir.length,
